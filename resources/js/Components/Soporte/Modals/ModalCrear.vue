...
const crearSoporte = async () => {
    try {
        const response = await axios.post("/soportes", nuevoSoporte.value, {
            headers: {
        ...
        if (error.response && error.response.status === 422) {
            ...
            console.error("Error al crear al soporte técnico:", error);
        }
    }
};
...
onMounted(() => {
    nuevoSoporte.value.password = generarPasswordAleatorio();
    fetchSedes();
});
</script>
...
<template>
    ...
    >
        <div class="w-full max-w-lg p-6 bg-white rounded-lg shadow-lg">
            <h2 class="mb-4 text-xl font-bold">Crear Nuevo Soporte Técnico</h2>
