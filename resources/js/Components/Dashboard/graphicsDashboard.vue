<template>
        ...
        <table class="min-w-full bg-white">
          ...
          <tbody>
            <tr v-for="ticket in tickets" :key="ticket.id" class="border-t">
              ...
              <td :class="{'text-green-500': ticket.estado === 'Abierto', 'text-red-500': ticket.estado === 'Cerrado'}" class="py-2">{{ ticket.estado }}</td>
  ...
  export default {
    ...
    computed: {
      // Datos para el gráfico de pastel de estado
      estadoChartData() {
          ...
          datasets: [
            {
              ...
              backgroundColor: ['#F87171', '#FDBA74', '#34D399'], // Rojo, naranja, verde
              borderColor: ['#E11D48', '#F97316', '#10B981'], // Colores para bordes
              borderWidth: 1
            }
          ]
        };
  ...
  /* Asegurarse de que los gráficos tengan dimensiones adecuadas */
  .chart-container {
    position: relative;
    height: 400px;
    width: 100%;
